...
-- Locate FastRenderer-Client import library
local fr_client_build_dir = path.join(os.projectdir(), "..", "FastRenderer-Client-Win", "build")
local fr_client_lib_dir

if is_mode("debug") then
    fr_client_lib_dir = path.join(fr_client_build_dir, "windows", "x64", "debug")
else
    fr_client_lib_dir = path.join(fr_client_build_dir, "windows", "x64", "release")
end

target("FRTest-Native")
    ...
    after_build(function(target)
        local bindir = path.join(os.projectdir(), "..", "bin", "FRTest-Native")
        os.mkdir(bindir)
        os.cp(target:targetfile(), bindir .. "/FRTest-Native.dll")
        os.cp(target:symbolfile(), bindir)
        os.cp(path.join(os.projectdir(), "manifest.json"), bindir .. "/manifest.json")
    end)
