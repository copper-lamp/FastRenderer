...
- **前置框架**：客户端插件通过 TCP 桥或文件热加载注册 GUI
- **多语言支持**：C++ 原生、LegacyScriptEngine (JS)、任意能写 JSON 文件的语言均可调用
- **跨平台**：Windows (D3D11/D3D12) + Android (OpenGL ES 3.0) 双端覆盖
- **独立运行**：FR Client 不依赖服务端，单机/局域网/联服均可使用
- **声明式 GUI**：JSON 定义界面，插件只需声明布局
- **热加载**：GUI 定义文件修改即时生效，无需重启
- **TCP 桥通信**：统一跨进程通信协议（JSON over TCP），替代 MC 自定义 Packet
- **服务端可选增强**：当服务端也有 FR 时，服务端插件通过 TCP 桥推送 UI 到客户端
...
│  │                                                         │    │
│  │  ┌─────────────────────────────────────────────────┐    │    │
│  │  │  [已废弃] 原 BridgeService 文件桥接               │    │    │
│  │  │  已由 TCP 桥接 (TcpClient) 替代                   │    │    │
│  │  │  参见 archive/deprecated/BridgeService.h          │    │    │
│  │  └─────────────────────────────────────────────────┘    │    │
│  │                                                         │    │
│  │  ┌─────────────────────────────────────────────────┐    │    │
│  │  │  共享核心层 (Core)                               │    │    │
...
│  │                                                         │    │
│  │  ┌─────────────────────────────────────────────────┐    │    │
│  │  │  网络层 (TCP Bridge) — 跨进程通信                   │    │    │
│  │  │  TcpClient → TCP Server (BDS)                     │    │    │
│  │  └─────────────────────────────────────────────────┘    │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐    │
...
| 层次 | 模块 | 职责 | 平台相关性 |
|------|------|------|-----------|
| **客户端通信** | ~~BridgeService~~ (已废弃) | 已由 TCP 桥接 (TcpClient) 替代，原代码归档于 `archive/deprecated/` | 通用 |
...
| **客户端网络** | TcpClient | 连接 Server TCP 端口，JSON 消息收发 + 自动重连 | 通用 |
| **服务端网络** | TcpServer | 内嵌在 FR-Server 中，管理客户端连接 + 消息广播/定向 | 服务器端 |
| **服务端插件** | Server PluginApi | 供服务端插件调用，通过 TCP Server 推送 UI 到客户端 | 通用 |
...
| 消息类型 (type) | 方向 | 用途 | 关键字段 |
|---|---|---|---|
| `identify` | Client→Server | 客户端身份标识（玩家名+平台类型） | `player`, `platform` |
| `gui_register` | Server→Client | 注册/更新 GUI 定义 | `pluginId`, `guiId`, `definition`, `targetPlayer` |
| `gui_unregister` | Server→Client | 卸载 GUI | `pluginId`, `guiId`, `targetPlayer` |
| `keybind_register` | Server→Client | 注册按键绑定 | `pluginId`, `bindId`, `bindName`, `vkCode`, `targetPlayer` |
| `keybind_unregister` | Server→Client | 卸载按键绑定 | `pluginId`, `bindId`, `targetPlayer` |
| `gui_event` | Client→Server | GUI 交互事件上报 | `guiId`, `controlId`, `eventType`, `value` |
| `data_exchange` | 双向 | 通用数据交换 | `channel`, `data`, `targetPlayer` |
...
```json
{"type":"gui_register","pluginId":"shop","guiId":"main_panel","definition":"{\"root\":{\"type\":\"window\",\"props\":{\"title\":\"商店\"}}}","targetPlayer":""}

{"type":"gui_event","guiId":"main_panel","controlId":"buy_btn","eventType":"click","value":""}
```
...
Client                                Server
  ...
  │  2. 身份标识                         │
  │  {"type":"identify",                 │
  │   "player":"Steve","platform":"win"} │  记录 player→conn 映射
  │  ───────────────────────────────→    │
  │                                      │
  │  3. 双向通信                         │
  │  ← gui_register / data_exchange ──  │
  │  ─ gui_event / data_exchange ────→  │
  │                                      │
  │  4. 断开                             │  清理映射
```
...
TCP Server 内嵌在 FastRenderer-Server 的 `enable()` 中，作为独立线程运行：

- 监听端口：默认 `12345`
- 多客户端管理：维护 `player→connection` 映射
- 消息路由：`targetPlayer=""` 广播，否则定向发送
- 心跳：30s 检测，60s 超时断开

### 3.5 与原有协议的对比

| 维度 | MC 自定义 Packet (原) | TCP 桥 (现) |
|---|---|---|
| 传输层 | RakNet (MC 内置) | TCP Socket |
| 载荷格式 | JSON (序列化到 MC Packet) | JSON (裸文本) |
| 客户端支持 | 仅 Win (LeviLamina) | Win + Android 统一 |
| Server 端插件代码 | 需已知 | 零改动（接口不变） |
| 端口管理 | 无需 | 额外 12345 端口 |
| 文件变更 | FRPackets.h / PacketBridge.h | 删除这两个文件 |
...
FastRenderer 提供多种通信方式，插件按需选择：

### 6.1 TCP 桥（主推）

统一跨进程通信协议（JSON over TCP），详见 [FR-TCP桥接通信协议.md](FR-TCP桥接通信协议.md)。

```
Server 端 LL 插件 ←→ FR-Server (TcpServer) ← TCP :12345 → FR-Client (Win/Android)
```

服务端插件调用 `IFastRendererAPI` 接口不变，底层自动转发为 TCP 消息。

### 6.2 文件热加载（离线备用）

`gui_defs/` 目录下的 JSON 文件自动加载，修改即时生效。
...
`keybinds/` 目录下的 JSON 文件在模组启动时自动注册按键绑定。

```json
{
    "keybinds": [
        {"pluginId": "my_plugin", "bindId": "open", "bindName": "打开面板", "vkCode": 79}
    ]
}
```
...
```
场景A: 服务端插件 → TCP 桥 → 客户端 (主路径)
  [服务端插件] → ServerPluginApi → TcpServer → 广播/定向 → [FR-Client]
                                                        → GuiService / KeybindService

场景B: 客户端本地插件 → 文件热加载 (离线)
  [客户端插件] → 写入 gui_defs/目录 JSON → [HotReloadService]
                                         → GuiService / KeybindService

场景C: 第三方语言脚本 → 文件热加载
  [JS/Python/Lua 插件] → 写入 gui_defs/目录 JSON → [HotReloadService]
...
- **内存注入优先级**：TCP 桥接和网络 Packet 注入的定义优先于文件定义
...
│   │   ├── DX11Hook.h
│   │   ├── TcpClient.h             ← TCP 客户端（Win/Android 复用）
...
│   ├── src/
│   │   ├── TcpServer.h             ← TCP Server（替代 FRPackets）
...
| 1.11 | TCP 桥接协议实现 (Server TcpServer + Client TcpClient) | ✅ |
| 1.12 | 删除文件桥 / PacketBridge，统一为 TCP 桥 | ✅ |
| 1.13 | FRTest-Native E2E 验证插件 (10 GUI 面板 + TCP 协议) | ✅ |
| 1.14 | RenderDragon 兼容 (D3D12 ExecuteCommandLists Hook) | ✅ |

### Phase 2: Android 移植

| 序号 | 任务 | 状态 |
|------|------|------|
| 2.1 | NDK CMake 项目搭建 + ImGui/nlohmann_json 集成 | ✅ |
| 2.2 | PL_REGISTER_MOD 插件入口，验证生命周期 | ✅ |
| 2.3 | GlossHook hook eglSwapBuffers + ImGui OpenGLES3 渲染 | ✅ |
| 2.4 | TcpClient 实现 + 身份标识 + 消息路由 | ✅ |
| 2.5 | PreloaderInput 触摸事件 → ImGui 鼠标事件映射 | ✅ |
| 2.6 | 虚拟按键面板 + 浮动按钮 | ✅ |
| 2.7 | 构建验证 (libFastRenderer-Android.so 编译成功) | ✅ |
...
2. **TCP 端口冲突**：BDS 服务器上需保证 TCP 端口（默认 12345）未被其他程序占用
...
6. **TCP 安全**：当前协议无加密，运行在可信网络中；外网环境建议配合 VPN 或防火墙
7. **RenderDragon 兼容**：部分 MCBE 版本 RenderDragon 实现差异可能影响 D3D11On12 路径
