...
target("FastRenderer-Server")
    ...
    after_build(function(target)
        local bindir = path.join(os.projectdir(), "..", "bin", "FastRenderer-Server")
        os.mkdir(bindir)
        os.cp(target:targetfile(), bindir .. "/FastRenderer-Server.dll")
        os.cp(target:symbolfile(), bindir)
        os.cp(path.join(os.projectdir(), "..", "FastRenderer-Server", "manifest.json"), bindir .. "/manifest.json")
    end)
